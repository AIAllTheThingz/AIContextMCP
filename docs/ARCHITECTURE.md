...
The local server runs beside `<dotnet-dev-checkout>` under one trusted Windows account.
...
Global registration is implemented for the configured client and the controlled Chunk 8 pilot is complete. Automated backup and organization-wide onboarding remain future work.
